...
class MQTTManager: ObservableObject {
    ...
    private func setupMQTTClient() {
        guard settings.enableMQTT,
              ...
              let port = UInt16(settings.mqttPort) else {
        ...
        client.willMessage = CocoaMQTTMessage(topic: willTopic, string: "offline", qos: .qos1, retained: true)
    ...
    private func publishBatterySensorDiscovery() {
        ...
        let config: [String: Any] = [
            ...
            "json_attributes_topic": "\(settings.mqttTopicPrefix)/sensor/\(deviceSerializedId)/battery/attributes",
            "device": deviceInfo,
            "availability": [
                [
                    "topic": "\(settings.mqttTopicPrefix)/binary_sensor/\(deviceSerializedId)/status",
    ...
    private func publishScreensaverButtonDiscovery() {
        ...
        let config: [String: Any] = [
            ...
            "command_topic": "\(settings.mqttTopicPrefix)/button/\(deviceSerializedId)/screensaver/set",
            "device": deviceInfo,
    ...
    private func publishAppInfoSensorDiscovery() {
        ...
        let config: [String: Any] = [
            ...
            "state_topic": "\(settings.mqttTopicPrefix)/sensor/\(deviceSerializedId)/app_info/state",
            "json_attributes_topic": "\(settings.mqttTopicPrefix)/sensor/\(deviceSerializedId)/app_info/attributes",
    ...
    private func startPeriodicUpdates() {
        ...
        batteryTimer = Timer.scheduledTimer(withTimeInterval: settings.mqttBatteryUpdateInterval, repeats: true) { _ in
    ...
    private func publishAppInfo() {
        ...
        let attributesTopic = "\(settings.mqttTopicPrefix)/sensor/\(deviceSerializedId)/app_info/attributes"
        publishJSON(topic: attributesTopic, payload: attributes)
    }
    ...
    private func publish(topic: String, payload: String, qos: CocoaMQTTQoS = .qos0, retain: Bool = false) {
...
extension MQTTManager: CocoaMQTTDelegate {
    ...
    func mqtt(_ mqtt: CocoaMQTT, didConnectAck ack: CocoaMQTTConnAck) {
        if ack == .accept {
            ...
            updateConnectionStatus("Verbindung abgelehnt: \(ack)")
        }
    }
    ...
    func mqtt(_ mqtt: CocoaMQTT, didDisconnectWith err: Error?) {
        ...
        if let error = err {
            updateConnectionStatus("Verbindung verloren: \(error.localizedDescription)")
